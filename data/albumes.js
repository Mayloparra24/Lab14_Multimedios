...
export const search = (text) => {
  const query = db.prepare(
    "SELECT slug FROM albumes WHERE titulo LIKE ? OR artista LIKE ? OR genero LIKE ?"
  );
  return query.all(`%${text}%`, `%${text}%`, `%${text}%`);
};

export const create = (album) => {
  const query = db.prepare(`INSERT INTO albumes (
...
export const update = (slug, album) => {
  const query = db.prepare(`UPDATE albumes SET
    titulo = :titulo,
    artista = :artista,
    genero = :genero,
    anio = :anio,
...
export const slugExists = (slug) => {
  const query = db.prepare("SELECT COUNT(*) as count FROM albumes WHERE slug = ?");
